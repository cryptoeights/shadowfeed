...
async function getCachedTvlData(kv: KVNamespace): Promise<Record<string, { tvl: number; change_1d: number }> | null> {
  const cached = await kv.get('defi_tvl', 'json') as Record<string, { tvl: number; change_1d: number }> | null;
  ...
  try {
    ...
    const protocols = await res.json() as Array<{ slug: string; tvl: number; change_1d: number | null }>;
    ...
    if (Object.keys(results).length > 0) {
      await kv.put('defi_tvl', JSON.stringify(results), { expirationTtl: 300 });
      return results;
    }
  } catch {}
  return null;
...
export async function generateDeFiScores(kv: KVNamespace) {
  const realTvl = await getCachedTvlData(kv);
